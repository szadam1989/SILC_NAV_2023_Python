...
file_save = 'Talalt_Parok_1._kat_g_PYTHON.txt'
...
for index, x in SILC.iterrows():
    ...
    if (len(SILC_Name) <= 2 or len(SILC_Mother_Name) <= 2):
        continue
    ...
    for ind, y in database_Subset.iterrows():
        ...
        Mother_Name_Diff = stringdist.levenshtein(" ".join([SILC_Mother_Name[1], SILC_Mother_Name[2]]), " ".join([y['AVNEVE'], MotherFirstName[0]]))
