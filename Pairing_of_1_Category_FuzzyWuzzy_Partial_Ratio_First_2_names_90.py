...
file_save = 'Talalt_Parok_1._kat_Partial_Ratio_First_2_names_90_PYTHON.txt'
...
for index, x in SILC.iterrows():
    ...
    for ind, y in database_Subset.iterrows():
        ...
        Employee_Name_Diff = fuzz.partial_ratio(" ".join([SILC_Name[0], SILC_Name[1]]), " ".join([y['VNEVEM'], FirstName[0]]))
        Employee_BirthName_Diff = fuzz.partial_ratio(" ".join([SILC_Name[0], SILC_Name[1]]), " ".join([y['SZVNEVE'], BirthFirstName[0]]))
        Mother_Name_Diff = fuzz.partial_ratio(" ".join([SILC_Mother_Name[0], SILC_Mother_Name[1]]), " ".join([y['AVNEVE'], MotherFirstName[0]]))
