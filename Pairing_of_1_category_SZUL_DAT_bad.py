...
file_save = 'Talalt_Parok_1._kat_SZUL_DAT_NEM_EGYEZIK_TOKEN_SET_100_PYTHON.txt'
...
for index, x in SILC.iterrows():
    ...
    if (x['Kategoria'] != 1 or x['Talalt'] != 0):
        continue
    #database['VNEVEM'].str.slice(0, 1) == x['SZNEV_VIZSGALT'][0:1] | 
    database_Subset = database.loc[(database['SZVNEVE'].str.slice(0, 1) == x['SZNEV_VIZSGALT'][0:1]) & (database['AVNEVE'].str.slice(0, 1) == x['ANYNEV_VIZSGALT'][0:1])]
    ...
    for ind, y in database_Subset.iterrows():
        ...
        if (Employee_Name_Diff == 100 and Mother_Name_Diff == 100):
